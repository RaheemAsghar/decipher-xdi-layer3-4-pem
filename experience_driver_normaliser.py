...
class GodTierCanonicalizer:
    ...
    def load_config(self, config_path: str) -> Dict[str, Any]:
        ...
        default_config = {
            ...
            # 🚫 BLOCKLIST - Known dangerous merges
            "blocklist_pairs": [
                ["Late delivery", "Damaged delivery"],
                ["Login issues", "Payment failures"],
                ["Account locked", "Account closed"],
                ["Billing error", "Service outage"],
                ["Product defect", "Shipping delay"],
                ["Password reset", "Account suspended"]
            ],
            
            # ✅ ALLOWLIST - Forced safe merges  
            "allowlist_pairs": [
                ["Delivery lateness", "Late delivery"],
                ["Payment processing", "Payment process"],
                ["Account access", "Login access"],
    ...
    def _clean_experience_driver(self, raw_ed: str) -> Optional[str]:
        """Enhanced Experience Driver cleaning with validation"""
    ...
    def _is_blocked_merge(self, ed1: str, ed2: str) -> bool:
        """Check if merge is blocked by safety rules"""
        blocklist = self.config.get("blocklist_pairs", [])
        
        for blocked_pair in blocklist:
            if len(blocked_pair) == 2:
                a, b = blocked_pair
                if (self._semantic_match(ed1, a) and self._semantic_match(ed2, b)) or \
                   (self._semantic_match(ed1, b) and self._semantic_match(ed2, a)):
                    self.stats["blocklist_hits"] += 1
                    return True
        return False
    
    def _is_allowed_merge(self, ed1: str, ed2: str) -> bool:
        """Check if merge is explicitly allowed"""
        allowlist = self.config.get("allowlist_pairs", [])
        
        for allowed_pair in allowlist:
            if len(allowed_pair) == 2:
                a, b = allowed_pair
                if {self._normalize_text(ed1), self._normalize_text(ed2)} == \
                   {self._normalize_text(a), self._normalize_text(b)}:
                    return True
        return False
    ...
    def god_tier_ed_match(self, theme: str, raw_ed: str, domain: str = "general") -> Dict[str, Any]:
        ...
        if dual_gate_passed and not is_blocked:
            # Safe to merge
        ...
        elif is_allowed and not is_blocked:
            # Force merge due to allowlist
            canonical_ed = proposed_canonical
            needs_review = False
            action = "force_merged"
            self.stats["merges_approved"] += 1
            ...
            # Preserve original + queue proposal
    ...
    def _queue_proposal(self, proposal: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Also add to in-memory queue for immediate access
    ...
    def batch_process(self, df: pd.DataFrame, domain: str = "general", 
        ...
        result_columns = [
            ...
            "canonical_category", "canonical_subcategory", "theme_match_score",
            "category_match_score", "subcategory_match_score", "needs_review",
            "action", "dual_gate_passed", "is_blocked", "is_allowed"
        ]
        
        for col in result_columns:
            df[col] = "" if col in ["canonical_theme", "canonical_experience_driver", 
                                  "canonical_ed_id", "canonical_category", 
                                  "canonical_subcategory", "action"] else (False if col in ["needs_review", "dual_gate_passed", "is_blocked", "is_allowed"] else 0)
        ...
        logging.info(f"✅ Batch processing complete: {processed_count} processed, {error_count} errors in {processing_time:.2f}s")
    ...
    def approve_proposal(self, proposal_id: str, action: str = "approve") -> bool:
        ...
        # Implementation for human-in-the-loop workflow
        try:
            proposals_df = self.export_proposals_report()
            if proposal_id in proposals_df.index:
                proposal = proposals_df.loc[proposal_id]
                
                if action == "approve":
                    # Execute the proposed merge
                    self._execute_proposal_merge(proposal)
                    logging.info(f"✅ Proposal {proposal_id} approved and merged")
                    return True
                    
                elif action == "reject":
                    # Keep original, mark as reviewed
