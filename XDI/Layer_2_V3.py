...
class Layer2Computer:
    ...
    def _derive_temporal_intelligence_from_tiers(self, by_tier: dict, all_tiers: list) -> dict:
        ...
        Thresholds are statistically grounded:
        - ERI: Tier-crossing logic based on YOUR defined tier boundaries
        - EVI: Tier-crossing logic based on YOUR defined EVI tier structure
        - Emotion Momentum: Statistical percentage thresholds (±5%, ±10%, ±20%)
        ...
        # Helper: ERI tier index (for tier-crossing calculation)
        def eri_tier_index(eri_value):
            """
            Map ERI value to tier index based on YOUR tier boundaries.
            Boundaries: -50, -10, 30, 80
            Returns: 0 (Very Negative), 1 (Negative), 2 (Neutral), 3 (Positive), 4 (Very Positive)
            """
            if eri_value < -50:
        ...
        # Helper: EVI tier index (for tier-crossing calculation)
        def evi_tier_index(evi_value):
            """
            Map EVI value to tier index based on YOUR tier boundaries.
            Boundaries: 20, 40, 60, 80
            Returns: 0 (Stable), 1 (Low Vol), 2 (Moderate Vol), 3 (High Vol), 4 (Extreme Vol)
            """
            if evi_value < 20:
        ...
        # --- ERI Trajectory (TIER-CROSSING LOGIC) ---
        ...
        if recent_eris and early_eris:
            ...
            start_tier_idx = eri_tier_index(eri_start)
            end_tier_idx = eri_tier_index(eri_end)
            tier_shift = end_tier_idx - start_tier_idx  # Signed distance

            # Statistically-grounded thresholds based on tier crossings
            if tier_shift >= 2:
            ...
            eri_trajectory = {
                ...
                "end": eri_end,
                "delta": eri_delta,
                "tier_shift": tier_shift,  # Number of tiers crossed
                "trend": eri_trend,
        ...
        # --- EVI Trajectory (TIER-CROSSING LOGIC) ---
        ...
        if recent_evis and early_evis:
            ...
            # Calculate tier-crossing distance
            start_tier_idx = evi_tier_index(evi_start)
            end_tier_idx = evi_tier_index(evi_end)
            tier_shift = end_tier_idx - start_tier_idx  # Signed distance
        ...
        # --- Emotion Momentum (STATISTICALLY-GROUNDED THRESHOLDS) ---
        ...
        for em in all_emotions:
            ...
            # Statistically-grounded thresholds (percentage point shifts)
            if delta >= 20:
        ...
        # --- Overall interpretation (ES Band migration priority) ---
    ...
    def _interpret_temporal_pattern(self, eri_traj, evi_traj, es_mig, emot_mom) -> str:
        ...
        if es_mig and es_mig.get("changed"):
            ...
            if "ES5" in es_start and "ES4" in es_end:
                return "SLIGHT IMPROVEMENT: Critical Failure → Active Crisis (still critical)"

        # PRIORITY 2: Combined ERI + EVI patterns (if no migration or no change)
