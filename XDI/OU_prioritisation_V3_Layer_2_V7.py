...
from OU_prioritisation_V3 import TemporalIntelligenceEnricher
...
class FlexibleTimeframeAnalyzer:
    ...
    def run_analysis(self):
        ...
        # Step 1: Compute unified Layer 2 (includes all ERI/EVI/RF + Temporal Intelligence)
        layer2 = Layer2Computer(
            df=self.raw_df,
            window_days=self.timeframe_days if self.date_range_type == 'window' else None,
            today_anchor=self.end_date,
            verbose=self.verbose
        )

        self.layer2_df = layer2.compute()
        
        # Layer 3 is now fully integrated into Layer 2
        self.layer3_df = self.layer2_df.copy()
        
        if self.verbose:
            print("✅ Unified Layer 2 computation complete")
            print(self.layer3_df.head())
        
        # Step 2: Add time window label
        if self.date_range_type == 'full':
        ...
        # Step 4: Save Layer 2/3 output to CSV
        output_path = "outputs/layer2_unified_output.csv"
        os.makedirs("outputs", exist_ok=True)
        self.layer3_df.to_csv(output_path, index=False, encoding="utf-8")
        print(f"✅ Unified output saved to: {output_path}")
                
        # Step 5: Save longitudinal SQLite (SEU-level)
        self._save_longitudinal_sqlite()

        # Step 6: Granular OU birthing + Temporal Intelligence filtering
        if self.compute_granular:
            # 🆕 Birth OUs (all candidates)
            self.details_df = self.compute_granular_details_snapshot(self.raw_df, self.layer3_df)
            
            # 🆕 Apply temporal intelligence filter
            if self.details_df is not None and not self.details_df.empty:
                self.details_df = self._apply_temporal_filter(self.details_df, self.layer3_df)
            
            # 🔥 NOW save to SQLite (with temporal decisions included)
        ...
        for ed, ed_ous in details_df.groupby("experience_driver"):
            ...
            for idx, ou_row in ed_ous.iterrows():
                ou_candidates.append({
                    ...
                    "dominant_emotion": ou_row.get("emotion_primary"),
