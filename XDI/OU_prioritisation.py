...
class TemporalIntelligenceFilter:
    ...
    def __init__(self):
        # Interpretation thresholds (tunable but not new metrics)
        self.RECENCY_THRESHOLD_HIGH = 60.0      # >60% in recent tiers = highly recent
        self.RECENCY_THRESHOLD_MODERATE = 40.0  # 40-60% = moderately recent
        self.RECENCY_THRESHOLD_LOW = 20.0       # <20% = historical/fading
        
        self.TIER_ABSENCE_THRESHOLD = 3         # Absent from top 3 tiers = disappeared
        self.MOMENTUM_STRONG_THRESHOLD = 15.0   # >15 pct point shift = strong momentum
        self.MOMENTUM_MODERATE_THRESHOLD = 8.0  # 8-15 pct point shift = moderate momentum
    ...
    def _check_pattern_context(
        ...
        # Simple keyword matching for crisis/negative patterns
        crisis_keywords = ["CRISIS", "COLLAPSE", "FAILURE", "DETERIORAT"]
        recovery_keywords = ["RECOVERY", "IMPROV", "STABILI"]
        
        is_crisis_pattern = any(kw in pattern_label.upper() for kw in crisis_keywords)
        is_recovery_pattern = any(kw in pattern_label.upper() for kw in recovery_keywords)
        
        # Negative emotions supported by crisis patterns
