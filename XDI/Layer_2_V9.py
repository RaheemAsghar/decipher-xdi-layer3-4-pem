...
class SEUCore:
    ...
    rf_weight_f: float
    rf: float
    rf_r_component: float
    rf_r_contribution_pct: float
    rf_f_component: float
    rf_f_contribution_pct: float
    rf_urgency_category: str
    rf_tier_short: str
    ...
    # Coverage
    associated_entity_names: List[str]
    no_of_mentions: int
    ...
    eri_rf_quadrant: Optional[str] = None
    quadrant_purpose: Optional[str] = None
    priority_status: Optional[str] = None
    quadrant_key: Optional[str] = None
...
class SEUMemory:
    ...
    tier_volume_profile: Dict[str, Any]
    tier_volume_summary: Dict[str, Any]
...
class SEU:
    core: SEUCore
    memory: Optional[SEUMemory] = None
    intelligence: Optional[SEUIntelligence] = None
    debug: Optional[Dict[str, Any]] = None

    def to_flat_record(self, include_memory: bool = False, include_intelligence: bool = False) -> Dict[str, Any]:
        rec = self.core.to_dict()
        # Keep legacy-compatible names if needed
        # Convert to legacy keys expected by prior CSVs
        legacy_map = {
            ...
            "rf": "RF",
            "rf_r_component": "RF_R_Component",
            "rf_r_contribution_pct": "RF_R_ContributionPct",
            "rf_f_component": "RF_F_Component",
            "rf_f_contribution_pct": "RF_F_ContributionPct",
            "rf_urgency_category": "RF_Urgency_Category",
            "rf_tier_short": "RF_Tier_Short",
            "priority_tier": "Priority_Tier",
            "priority_purpose": "Priority_Purpose",
            "associated_entity_names": "Associated_Entity_Names",
            "no_of_mentions": "No_of_Mentions",
            "mention_share_pct": "Mention_Share_%",
            "first_seen_date": "First_Seen_Date",
            "most_recent_date": "Most_Recent_Date",
            "age_days": "Age_Days",
            "eri_rf_quadrant": "ERI_RF_Quadrant",
            "quadrant_purpose": "Quadrant_Purpose",
            "priority_status": "Priority_Status",
            "quadrant_key": "Quadrant_Key",
        }
        out = {"experience_driver": rec.get("experience_driver")}
        for k, v in rec.items():
            if k == "experience_driver":
                continue
            out[legacy_map.get(k, k)] = v

        if include_memory and self.memory is not None:
            out["SEU_Memory"] = self.memory.to_dict()
        if include_intelligence and self.intelligence is not None:
            out["SEU_Intelligence"] = self.intelligence.to_dict()
        if self.debug is not None:
            out["SEU_Debug"] = self.debug
        return out
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.intelligence is not None:
            d["intelligence"] = self.intelligence.to_dict()
        if self.debug is not None:
            d["debug"] = self.debug
        return d
...
class Layer2Computer:
    ...
    PRIORITY_GRID = {
        ...
        "ES3_At_Risk": {
            "Dormant":  "P2",  # 🔥 FIXED: Silent risk = urgent investigation (was P3)
            "Low":      "P3",
            "Moderate": "P2",
            "High":     "P1",
            "Very High":"P1",
        ...
        "ES5_Critical_Failure": {
            "Dormant":  "P2",  # Reputation/containment risk
            "Low":      "P2",
            "Moderate": "P1",
            "High":     "P0",
            "Very High":"P0",
    ...
    PURPOSE_MATRIX = {
        ...
        ("ES3_At_Risk", "P3"): "Investigate Silent Risk",      # Low RF only
        ("ES3_At_Risk", "P2"): "Address Early Warning Signs",  # 🔥 UPDATED: Now covers Dormant + Moderate
    ...
    def compute(self, return_format: str = "dataframe", include_memory: bool = False, include_intelligence: bool = False):
            """
            Layer 2 (ERI + EVI + RF):
            ...
            for entity, group in grouped:
                ...
                priority_purpose = self._classify_purpose(emotional_state_band, priority_tier)
                                ...
                                # --- build layered SEU object --------------------------------------
                rec_profile = self._compute_emotion_recency_profile(group)
                ...
                if isinstance(rec_profile, dict):
                    temporal_block = rec_profile.get("temporal_intelligence")
                    memory_block = {k: v for k, v in rec_profile.items() if k != "temporal_intelligence"}

                # Associated entities & coverage
                associated_names = (
                ...
                core = SEUCore(
                    ...
                    rf_r_contribution_pct=rf_r_pct,
                    rf_f_component=round(rf_f_abs, 2),
                    rf_f_contribution_pct=rf_f_pct,
                    ...
                    mention_share_pct=mention_share_pct,
                ...
                if isinstance(memory_block, dict):
                    memory_obj = SEUMemory(
                        by_emotion=memory_block.get("by_emotion", {}),
                        by_tier=memory_block.get("by_tier", {}),
                        tier_volume_profile=memory_block.get("tier_volume_profile", {}),
                        tier_volume_summary=memory_block.get("tier_volume_summary", {}),
                    )
                else:
                    memory_obj = None

                if isinstance(temporal_block, dict):
                    intelligence_obj = SEUIntelligence(temporal_intelligence=temporal_block)
                else:
                    intelligence_obj = None

                seu = SEU(
                    core=core,
                    memory=memory_obj,
                    intelligence=intelligence_obj,
                    debug={"Debug_Emotion_Counts": emotion_counts},
                )

                seu_objects.append(seu)
                flat_results.append(
                    seu.to_flat_record(
            ...
            # Build DF once
            layer2_df = pd.DataFrame(flat_results)
            ...
            if return_format == "objects":
                return seu_objects
            if return_format == "both":
                return self.layer2_df, seu_objects
            raise ValueError("return_format must be one of: 'dataframe', 'objects', 'both'")
    ...
    def _compute_emotion_flow(
        ...
        - polarity_shift: Tracks aggregate positive/negative emotion group flows
        ...
        - polarity_flow_direction: "Negative → Positive" style categorical label
        ...
        - shifts: per-emotion delta (existing momentum logic)
        - dominant_shift: emotion with largest absolute delta
        - top_early_emotions: top 2-3 emotions in early period
        - top_recent_emotions: top 2-3 emotions in recent period
        - ambivalence_type: "true", "false", or "none"
        """
        # Aggregate percents for early vs recent vs combined
        ...
        # If we have no mentions in either region, bail out
        if not early_dist and not recent_dist:
            return None

        # Existing momentum logic for per-emotion deltas
        ...
        top_early = _top_k(early_dist)
        top_recent = _top_k(recent_dist)

        # True vs False ambivalence classification
        ...
        # Aggregate group percentages for EARLY period
        ...
        # Aggregate group percentages for RECENT period
        ...
        # Determine polarity flow direction (categorical label)
        # Use 15-point threshold for "meaningful" shifts
        threshold = 15.0
        ...
        # ========================================================================
        # NEW: DOMINANT EMOTION TRANSITION
        # ========================================================================
        
        # Identify dominant (most common) emotion in each period
        if early_dist:
        ...
        return {
            ...
            # NEW: Polarity tracking
            "polarity_shift": polarity_shift,
            
            # NEW: Dominant emotion tracking
