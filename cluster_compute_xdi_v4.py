...
class FlexibleTimeframeAnalyzer:
    def __init__(self, input_path, output_dir="outputs", timeframe_days=75, compute_granular=True, verbose=True):
       ...
       self.OU_CFG.setdefault("signature_weights", {
            "matters": 6, "context": 3, "interaction_moment": 2,
    ...
    def compute_feedbacktype_focus(self):
        """
        Compute dominant feedback types and their distribution per experience_driver
        (80% cumulative threshold logic applied).
        Replaces compute_emotional_focus().
        """

        df = self.raw_df
        valid_entities = set(self.layer3_df["experience_driver"].unique())

        # Canonical Feedback Type mapping (case-insensitive exact match)
        canonical_map = {
        ...
        def get_dominant_feedback_types(group_df, threshold: float = 0.80):
            mapped = (
                group_df["feedback_type"]
                .dropna()
                .str.strip()
                .str.lower()
                .map(canonical_map)
            ...
            # Dominant types until threshold reached
            dominant = []
            cumulative = 0.0
        ...
        grouped = filtered_df.groupby("experience_driver").apply(get_dominant_feedback_types)

        # Avoid inserting duplicate column on reset_index
    ...
    def _collapse_locator_composite(self, grp: pd.DataFrame):
        ...
        for j, s, m in zip(j_raw, s_raw, m_raw):
            jn, sn, mn = _norm_ws(self, j).lower(), _norm_ws(self, s).lower(), _norm_ws(self, m).lower()
            if not (jn or sn or mn):
                continue
            key = (jn, sn, mn)
            key_counts[key] += 1
            key_variants.setdefault(key, []).append((
                _norm_ws(self, j), _norm_ws(self, s), _norm_ws(self, m)
            ))
    ...
    def _make_root_cause_preview(self, grp: pd.DataFrame) -> str:
        ...
        mat = _mode_or_centroid(self, grp.get("matters", pd.Series(dtype=object)).astype(str).tolist()) or ""
        ctx = _mode_or_centroid(self, grp.get("context", pd.Series(dtype=object)).astype(str).tolist()) or ""
        mech = (mat or ctx).strip().rstrip(" .")

        # choose one valid locator combo from the data (no hardcoding)
        journey, stage, moment, _ = _collapse_locator_composite(self, grp)
        tail = _format_location_tail(self, journey, stage, moment)

        base = mech if mech else (journey or stage or moment or "No preview available")
        preview = _sentence_case(self, (base + tail).strip())
    ...
    def compute_granular_details_snapshot(self, raw_df: pd.DataFrame, layer3_df: pd.DataFrame) -> pd.DataFrame:
        """
        Build OU composites per (Experience Driver → Feedback Type → Stream) and persist clusters.
        ...
        for _, hdr in layer3_df.iterrows():
            ...
            # parse feedback type focus (list)
            ft_focus = hdr.get("feedbacktype_audit_focus", [])
            if isinstance(ft_focus, str):
                try:
                    ft_focus = ast.literal_eval(ft_focus)
                except (ValueError, SyntaxError):
                    ft_focus = []
            if not isinstance(ft_focus, (list, tuple, set)):
                ft_focus = []

            # feedback type distribution map (optional, for enrichment)
            ...
            # per feedback type in focus
            for ftype in ft_focus:
                ft_key = str(ftype).strip().lower()
                if not ft_key:
                    continue
                ...
                # streams for this feedback type
                stream_series = (
                ...
                for stream in dominant_streams:
                    ...
                    clust_df, full_distribution, cluster_store, df_chunk, full_composites = self.cluster_behavior(
                        stream_rows, driver=driver, feedback_type=ftype, stream=stream
                    )
