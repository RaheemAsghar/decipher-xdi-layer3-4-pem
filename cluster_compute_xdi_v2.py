   ...
   # === PEM (Predictive Emotional Modeling) — PATTERN-FUSED (balanced) ===
    def build_predictive_emotional_modeling(
        ...
        *,
        ...
        - Momentum strength from momentum_contract() via mom_details()
        ...
        # --- QSSI strength (consume numeric score only; no tier mapping) ---
        qssi_score = (
            state_of_play.get("qssi_score",
                state_of_play.get("qssi_summary", {}).get("qssi_score"))
        )
        qssi_strength = float(qssi_score) / 10.0 if qssi_score is not None else 0.0  # normalized 0..1

        # --- Momentum via helpers (no hardcoding) ---
        sym = momentum_symbol or self.mom_symbol_from_label(mom_label)
        sym = sym if sym in {"↑↑","↑","→","↓","↓↓"} else "→"
        mom_meta = self.mom_details(sym)
        strength = float(mom_meta.get("strength", 0.3))
        sign = 1.0 if sym.startswith("↑") else (-1.0 if sym.startswith("↓") else 0.0)
        dir_score = sign * strength  # ↑↑=>+1.0, ↑=>+0.6, →=>0.0, ↓=>-0.6, ↓↓=>-1.0

        # --- Saturation via helpers (no headroom map) ---
        ...
        for th, (emoji, clean), headroom, qssi_sc in self.saturation_contract():
            if clean == sat_tier_clean:
                ...
                break
        if qssi_score_for_tier is None:  # fallback to neutral contract bin
        ...
        esc_raw = (
            ...
            0.30 * (1.0 - headroom_scalar) +  # more headroom (closer to Champion) => larger (1 - scalar)
            0.25 * stability
        )
        dec_raw = (
            0.35 * qssi_strength * max(0.0, -dir_score) +
            0.55 * max(0.0, -dir_score) +
            0.30 * headroom_scalar +          # closer to At-Risk increases decay propensity
            0.20 * vol_norm
        )
        ...
        # --- Coverage & stability caps (data-aware) ---
        if signal_presence_pct is None:
            presence_factor = 1.0
        else:
            presence = float(signal_presence_pct)
        ...
        # score-based cap (no tier mapping): 0.50..0.95 from qssi_score
        ...
        base_elasticity = (
            "High"     if (1.0 - headroom_scalar) >= 0.5 and abs(dir_score) >= 0.6 else
        ...
        return {
            ...
            "signal_diagnostics": {
                ...
                "qssi_tier": qssi_tier,
                "headroom_contract": headroom_dict,  # canonical narrative
            },
            "future_risk_profile": {
