...
class Layer3Computer:
    ...
    *,
    ...
    min_presence_gate: float = 0.05
        ...
        - Labels/descriptions are looked up from momentum_contract() via mom_details()
        - Presence gate: if signal_presence_pct < 5%, force Stable
        Returns: {"symbol","delta","label","description", "snr", "n_days"}
        """
        try:
            s = pd.Series(series).astype(float)
            s = s.replace([np.inf, -np.inf], np.nan).dropna()
            ...
            if n < 4:
                ...
                return {"symbol": "→", "delta": 0.0, "label": meta["label"], "description": meta["description"], "snr": 0.0, "n_days": n}
            ...
            return {
                ...
                "snr": snr_r,          # extra: harmless for downstream
                "n_days": n            # extra: harmless for downstream
            }
            ...
            meta = self.mom_details("→")
            return {"symbol": "→", "delta": 0.0, "label": meta["label"], "description": meta["description"], "snr": 0.0, "n_days": 0}
    ...
    *,
    ...
    momentum_symbol,
    *,
    # NEW (optional) context to make confidence truthful on sparse/unstable series
    ...
    series_data_complete: bool | None = None    # provenance flag (optional)
        ...
        using the class contracts for consistency + confidence that respects presence/volatility/SNR.
        """
        ...
        # 4) Tier-aware centrality, borderline, and confidence components
        ...
        # centrality within the tier (0 at boundary → 1 at the center)
        sat_centrality = float(min(1.0, sat_distance / half_width))

        # adaptive borderline: near a tier edge relative to bin width
        borderline = sat_centrality < min(0.15, 0.5 * half_width)

        # confidence components (all in 0..1)
        presence = None if signal_presence_pct is None else float(signal_presence_pct)
        presence_factor = 1.0 if presence is None else max(0.3, min(1.0, presence / 0.40))  # full credit ≥40% presence
        stability_factor = 1.0 if vol_norm is None else max(0.0, min(1.0, 1.0 - float(vol_norm)))  # stable → 1
        snr_factor = 1.0 if momentum_snr is None else max(0.5, min(1.0, float(momentum_snr) / 1.25))  # hit 1.0 at strong gate

        # base confidence: balanced blend (weights sum to 1)
        base_conf = (
            0.40 * mom_strength +       # how decisive the momentum class is
            0.35 * sat_centrality +     # how deep inside the tier we are
            0.25 * stability_factor     # penalize volatility
        )

        # apply data-quality caps
        conf = base_conf * presence_factor * snr_factor
        if series_data_complete is False:
            conf *= 0.9  # slight haircut for heavily inferred series
        ...
        payload["meta"] = {
            ...
            # extras for audit (harmless to keep)
            "components": {
    ...
    def _compute_signal_strength(
        ...
        *,
        ...
        # presence: full credit at >= 40% days with signal; floor to 0.3 to avoid zeroing
        if signal_presence_pct is None:
            presence_factor = 1.0
        else:
            presence = float(signal_presence_pct)
            presence_factor = max(0.3, min(1.0, presence / 0.40))
        ...
        # momentum SNR: hit 1.0 near your strong gate (≈1.25); floor at 0.5
        if momentum_snr is None:
            snr_factor = 1.0
        else:
            snr = float(momentum_snr)
            snr_factor = max(0.5, min(1.0, snr / 1.25))

        # length penalty (as before)
        penalty = 0
        ...
        # scale with momentum strength (decisiveness) and data quality
        velocity_scaled = (
            velocity_symbolic
        ...
        # keep final velocity in 0..6 int for your composite
        velocity_score = int(round(max(0.0, min(6.0, velocity_scaled))))
        ...
        return {
            ...
            "qssi_summary": {
                ...
                "dominant_driver": dominant_driver  # NEW, harmless/optional
            },
            "audit": {
                ...
                **({"vol_norm": float(vol_norm)} if vol_norm is not None else {}),
                # NEW: expose factors for transparency
                "factors": {
   ...
   # === PEM (Predictive Emotional Modeling) — PATTERN-FUSED (balanced) ===
    def build_predictive_emotional_modeling(
        ...
        qssi_tier,
        *,
        # Optional context (backward compatible)
        ...
        """
        Predictive Emotional Modeling (PEM) — fused with pattern recognition.
        - Uses recurrence (type/strength/pain_day) to tune horizon, probability & narrative.
        - Backward compatible keys; adds harmless extras inside nested dicts.
        """
        ...
        # Momentum (symbol & signed direction)
        sym = momentum_symbol or self.mom_symbol_from_label(mom_label)
        sym = sym if sym in {"↑↑","↑","→","↓","↓↓"} else "→"
        dir_score = {"↑↑":1.0, "↑":0.6, "→":0.0, "↓":-0.6, "↓↓":-1.0}[sym]

        # Headroom from saturation tier
        headroom_map = {"Very High":0.00, "High":0.25, "Medium":0.50, "Low":0.75, "Very Low":1.00}
        headroom = float(headroom_map.get(sat_tier, 0.50))

        # Pattern strength (numeric 0..1) from confidence/strength
        pat_conf = (pattern_confidence or "").lower()
        if pattern_strength is not None:
            pat_strength = float(max(0.0, min(1.0, pattern_strength)))  # normalize if user passes 0..1-ish
            # if user passed ACF like 0.29, lift lightly to same scale
            if pat_strength <= 1.0 and pat_strength <= 0.6:  # treat as ACF, map 0..0.6 → 0..1.0
                pat_strength = min(1.0, pat_strength / 0.6)
        else:
            pat_strength = 0.0
            if   pat_conf == "strong":   pat_strength = 1.0
            elif pat_conf == "moderate": pat_strength = 0.6
            elif pat_conf == "weak":     pat_strength = 0.3
        ...
        # Pattern-direction fusion: nudge probabilities slightly toward momentum direction if a pattern exists
        # (Keeps integrity; avoids overfitting.)
        if has_pat and pat_strength > 0:
            # scale: small boost 0..10% depending on strength
            ...
            if dir_score > 0:
                ...
                # flat momentum: very light bidirectional nudge to avoid false flipping
        ...
        cap_by_qssi = {
            "❌ No Signal":      0.55,
            "🔁 Weak Signal":    0.65,
            "🌱 Emerging Signal":0.75,
            "🔥 Strong Signal":  0.90,
            "💥 Critical Signal":0.95
        }
        tier_cap = cap_by_qssi.get(qssi_tier, 0.70)

        momentum_factor = 0.85 if sym == "→" else 1.0
        quality_cap = tier_cap * presence_factor * (0.8 + 0.2 * stability) * momentum_factor
        ...
        # === Rules-first overrides (momentum & pattern narrative) ===
        signal_is_precursor = (dir_score > 0 and qssi_tier in {"🔥 Strong Signal","💥 Critical Signal"} and has_pat)
        ...
        if signal_is_precursor:
            ...
            if max(esc_prob, dec_prob) < 0.55:
                ...
                risk_class      = "Neutral" if not has_pat else "Risk"
                if has_pat:
        ...
        # === Elasticity (pattern can bump one tier if strong) ===
        base_elasticity = (
            "High"     if headroom >= 0.5 and abs(dir_score) >= 0.6 else
            "Moderate" if headroom >= 0.25 else
            "Low"
        )
        if has_pat and pat_strength >= 0.6:
            elasticity_rating = "High" if base_elasticity == "Moderate" else base_elasticity
        else:
            elasticity_rating = base_elasticity
        ...
        # === Return (schema-compatible; adds a few safe fields) ===
        return {
            "trajectory_forecast": {
    ...
    def compute(self):
        ...
        for _, row in entities.iterrows():
            try:
                ...
                # ---- PEM (use adjusted volatility under sparse/incomplete + pass context) ----
                ...
                pattern_payload = {
                    ...
                    "pattern_strength": (
                        ...
                        if pattern_block["has_pattern"] and pattern_block.get("pattern_strength") is not None
                        else None
                    ),
